      ...
      <template v-if="extColumnsArrs.length > 0">
        <template v-for="extColumnsArr in extColumnsArrs">
          <template v-if="staff[extColumnsArr] != 0 ">
            ...
            <p class="payroll--right">{{ staff[extColumnsArr] }}</p>
          </template>
        </template>
      </template>
      <p class="payroll--left">小計</p>
      <p class="payroll--right">{{ subtotal }}</p>
    </div>
    <div class="payroll__insurance">
        <p class="payroll--left">勞保</p>
        <p class="payroll--right">{{ staff['勞保費'] }}</p>
        <p class="payroll--left">健保</p>
        <p class="payroll--right">{{ staff['健保費'] }}</p>
        <template v-if="staff['仲介費'] != 0">
          <p class="payroll--left">仲介費</p>
          <p class="payroll--right">{{ staff['仲介費'] }}</p>
        </template>
        
        <p class="payroll--left">總計</p>
        <p class="payroll--right">{{ total }}</p>
      </div>
...
<script>
  export default {
    ...
    computed: {
      ...
      total () {
        return this.subtotal + this.toNum(this.staff['勞保費']) + this.toNum(this.staff['健保費']) + this.toNum(this.staff['仲介費'])
      },
      replaceCommit () {
